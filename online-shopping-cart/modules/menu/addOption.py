...
def addItemToCartMenu(cart):
    ...
    def addItem():
        _cart = cart
        _item = item()
        _item.setItemName(str(e1.get()))
        _item.setItemDescription(str(e2.get()))
        _item.setItemPrice(float(e3.get()))
        _item.setItemQuantity(int(e4.get()))
    ...
    b2=tk.Button(window,text="Back",font=("Arial",13),command=window.destroy)
